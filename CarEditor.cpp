...
BOOL CCarEditorApp::InitInstance()
{
	...
	SetRegistryKey("GP3CarEditorProject\\CarEditor");
...
void CCarEditorApp::OnEditLocation() 
{
	...
	browser.lpszTitle = "Select GP3 Location";
	browser.lpfn = NULL;
	...
	if (files !=NULL)
	{
		...
		if (SHGetPathFromIDList(files,filename))
		{
         ...
         CString strStringItem    = "GP3Location";
		...
		{
	     AfxMessageBox("Failed to Locate GP3 directory");
		}
   	}
	else
	{
		AfxMessageBox("Failed to find GP3 directory");
	}
}
...
CString CCarEditorApp::getGP2Location()
{
	 ...
	 CString circuitsDir = pApp->GetProfileString(strSection, strStringItem,"c:\\gp3\\");
	 return circuitsDir;
}
