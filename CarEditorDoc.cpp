...
BOOL CCarEditorDoc::OnNewDocument()
{
	...
	wsprintf(buffer,"%s\\gp3.exe",gp2Location);
...
char lpszFileFilter[] = "F1GP3 Car Files (*.cs3)|*.cs3|All Files (*.*)|*.*|";
...
void CCarEditorDoc::OnFileSaveAs() 
{
	if (car)
	{
	 ...
	 strcpy(fileName,"unnamed.cs3");
	 CFileDialog *fdlg = new CFileDialog(FALSE,"cs3",(LPCSTR)&fileName,OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT,lpszFileFilter);
	 ...
	 int tmp_offset = 94;
	 car->exportCar(start,tmp_offset);
	 car->write(filename);
	 }
	}
}
...
char lpszFileHelmetFilter[] = "F1GP3 Helmet Files (*.hlt)|*.hlt|All Files (*.*)|*.*|";
...
char lpszFileGPOFilter[] = "F1GP3 Object Files (*.gpo)|*.gpo|All Files (*.*)|*.*|";
